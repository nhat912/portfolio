...
>(({ className, text = '', hasIcon = true, ...props }, ref) => {
  ...
  return (
    <button
      ref={ref}
      className={cn(
        "bg-gradient flex gap-x-2 items-center border-none !disabled:opacity-100",
        className
      )}
    ...
    >
      {canScrollPrev && (
        <>
          ...
          <span className="font-semibold text-xs lg:text-lg">{text || 'Previous slide'}</span>
        </>
      )}
    </button>
  )
